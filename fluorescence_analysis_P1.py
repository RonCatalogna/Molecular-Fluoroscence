...
# Final formatting
ax_main.set_xlabel("Concentration (mM)", fontweight='bold')
ax_main.set_ylabel("Normalized Intensity (Arb. Units)", fontweight='bold')
ax_main.set_title("Fluorescence Intensity vs. Concentration", pad=35)
...
for material, style in style_config.items():
    # Create a handle that clearly shows both line style and marker
    handle = Line2D([0], [0],
                    color=style["color"],
                    marker=style["marker"],
                    linestyle=style["linestyle"],
...
fig_main.tight_layout(rect=[0, 0, 1, 0.92])
