...
def integrating_data(filename, sheet, exposure_row, exposure_col): #Integration function
    ...
    return np.trapezoid(intensity, wavelength) / exposureTime
...
def data_process_RawData(filename):
    ...
    for sheet in xls.sheet_names:
        Int_i = integrating_data(filename, sheet, exposure_row, exposure_col)
        Con.append(float(sheet))
        Int.append(Int_i)

    Con, Int = np.array(Con), np.array(Int)
    sorting = np.argsort(Con)
    return Con[sorting], Int[sorting]
...
for material, file in files.items():
    ...
    Concentration_all, Intensity_all = data_process_RawData(file)
